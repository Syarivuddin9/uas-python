...
def insertDataMysql():
    ...
    if response.status_code == requests.codes.ok:
        ...
        for out in data['data']:
            ...
            alamat = out['alamat']
            cur = db.cursor()
            cur.execute('INSERT INTO `tbl_students_0584` (id, nim, nama, jk, jurusan, alamat) VALUES (null, %s, %s, %s, %s, %s)', (id, nim, nama, jk, jurusan, alamat))
            db.commit()

def showAllData():
    clear()
    cur = db.cursor()
    cur.execute('SELECT * FROM `tbl_students_0584`')
    data = cur.fetchall()
    print('+-----+--------------+----------------------------+----+-----------------------+----------------+')
...
def limitData():
    ...
    cur = db.cursor()
    cur.execute(f'SELECT * FROM `tbl_students_0584` LIMIT {x}')
    data = cur.fetchall()
    print('+-----+--------------+----------------------------+----+-----------------------+----------------+')
    print('| No. |     NIM      |             Nama           | JK |        Jurusan        |      Alamat    |')
    print('+-----+--------------+----------------------------+----+-----------------------+----------------+')
...
def limitNim():
    ...
    cur = db.cursor()
    cur.execute(f'SELECT * FROM `tbl_students_0584` WHERE nim = "{x}" LIMIT 1')
    data = cur.fetchall()
    print('+-----+--------------+----------------------------+----+-----------------------+----------------+')
    print('| No. |     NIM      |             Nama           | JK |        Jurusan        |      Alamat    |')
    print('+-----+--------------+----------------------------+----+-----------------------+----------------+')
